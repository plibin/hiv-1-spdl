...
# Positional All-Atom RMSD calculation script for all proteins.
# Iterates over each protein in config.py, runs cli-pos.py with --stat aa-rmsd,
# and writes results to CSV and warnings to log files in the results directory.
# Usage: ./cli-pos-aa-rmsd.sh
...
echo "Starting positional All-Atom RMSD calculations for all proteins..."
echo ""
...
for protein in "${proteins[@]}"; do
    ...
    output_csv="$results_dir/${protein_lower}-pos-aa-rmsd.csv"
    output_log="$logs_dir/${protein_lower}-pos-aa-rmsd.stderr.log"
...
echo "All positional All-Atom RMSD calculations complete."
