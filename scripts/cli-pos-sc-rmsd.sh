...
# Positional sidechain RMSD calculation script for all proteins.
# Iterates over each protein in config.py, runs cli-pos.py with --stat sc-rmsd,
# and writes results to CSV and warnings to log files in the results directory.
# Usage: ./cli-pos-sc-rmsd.sh
...
echo "Starting positional sidechain RMSD calculations for all proteins..."
echo ""
...
for protein in "${proteins[@]}"; do
    ...
    output_csv="$results_dir/${protein_lower}-pos-sc-rmsd.csv"
    output_log="$logs_dir/${protein_lower}-pos-sc-rmsd.stderr.log"
...
echo "All positional sidechain RMSD calculations complete."
